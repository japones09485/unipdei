...
export class UsuarioComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.acRouter.params.subscribe(param => {
      ...
      if (this.id != 0) {
        this.edit = true;
        this.auth.get_usuario(this.id)
          .subscribe((res: any) => {
            this.UsuarioData = res.data;
            this.banderaPais(this.UsuarioData.usu_pais);
            this.infoForm();
  ...
  infoForm() {
    ...
    this.frmCr = this.fb.group({
      identificacion: [this.UsuarioData.usu_identificacion, Validators.required],
      nombre: [this.UsuarioData.usu_nombres, Validators.required],
      apellidos: [this.UsuarioData.usu_apellidos, Validators.required],
      email: [this.UsuarioData.usu_email, [Validators.required, Validators.email]],
      telefono: [this.UsuarioData.usu_telefono, Validators.required],
      estado: [this.UsuarioData.usu_estado, Validators.required],
      pais: [this.UsuarioData.usu_pais, Validators.required],
    });
  ...
  guardarUsuario() {
    ...
    this.auth.guardarUsuario(this.frmGuardar)
      .subscribe((res: any) => {
        ...
        if (res.ok === true) {
          ...
          this.router.navigate(['/AdminUsuario/' + this.UsuarioData.usu_id]);
        }
      });
  }
